...
fn test_sub_dir() {
    ...
    let Ok(current_dir) = env::current_dir() else {panic!("No se pudo obtener el directorio actual")};
    let absolute_path = format!("{}/{}",current_dir.display(),path);
    _ = fs::remove_dir_all(format!("{}", absolute_path));
    let Ok(_) = fs::create_dir_all(absolute_path.clone()) else {panic!("No se pudo crear el directorio")};
    ...
    if !directories_exists(&format!("{}/{}/.git",absolute_path, "new_dir".to_string())){panic!("No se pudo obtener el directorio actual")};

    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let aux = format!("Initialized empty Git repository in {}/./new_dir\n", absolute_path);
...
fn test_init() {
    ...
    if !directories_exists(&format!("{}/.git",absolute_path)){panic!("No se pudo obtener el directorio actual")};
    if !files_exists(&format!("{}/.git",absolute_path)){panic!("No se pudo obtener el directorio actual")};
...
fn test_bare() {
    ...
    if !directories_exists(&format!("{}",absolute_path)){panic!("No se pudo obtener el directorio actual")};
    if !files_exists(&format!("{}",absolute_path)){panic!("No se pudo obtener el directorio actual")};
    if output.status.success() {
        let stdout = String::from_utf8_lossy(&output.stdout);
        let aux = format!("Initialized empty Git repository in {}\n", absolute_path);
        assert_eq!(stdout, aux);
