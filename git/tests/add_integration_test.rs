    ...
    match fs::File::open(format!("{}/.git/index", path)) {
        ...
        Ok(mut file) => match StagingArea::read_from(&mut file) {
            Ok(stagin_area) => assert_eq!(
                stagin_area.get_changes().get("testfile.txt").unwrap(),
...
fn test_two_files_in_dir() {
    ...
    match fs::File::open(format!("{}/.git/index", path)) {
        ...
        Ok(mut file) => match StagingArea::read_from(&mut file) {
            Ok(stagin_area) => {
                assert_eq!(
                    stagin_area.get_changes().get("dir/testfile1.txt").unwrap(),
                    "30d74d258442c7c65512eafab474568dd706c430"
                );
                assert_eq!(
                    stagin_area.get_changes().get("dir/testfile2.txt").unwrap(),
                    "30d74d258442c7c65512eafab474568dd706c430"
                )
            }
            Err(error) => panic!("No se pudo leer el staging area: {:?}", error),
