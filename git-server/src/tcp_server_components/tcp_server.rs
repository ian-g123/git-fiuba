...
impl TcpServer {
    pub fn start_server(address: &str, path: &str) -> Result<TcpServer, CommandError> {
        println!("Starting TCP server at {}...", address);
        ...
        let listener_handle = thread::spawn(move || {
            let logger = match Logger::new("tcp-server-logs.log") {
                Ok(logger) => logger,
                Err(error) => {
